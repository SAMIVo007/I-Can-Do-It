...
import { View, ScrollView, Switch, type ViewStyle } from "react-native";
...
export default function OnboardingScheduleScreen() {
  ...
  const { userName, goalTitle, focusArea, selectedHabits, reset } =
    useOnboardingStore();
  const { addGoal, addHabit } = useHabitStore();
  const [enableRemindersToggle, setEnableRemindersToggle] = useState(true);
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      if (enableRemindersToggle) {
        try {
          remindersOn = await requestNotificationPermissions();
          storage.set("remindersEnabled", remindersOn);
        } catch (permError) {
      ...
      if (remindersOn) {
        await syncAllHabitReminders(createdHabits);
      }
        ...
        <Animated.View entering={FadeInDown.duration(500).delay(200)}>
          <Card variant="bordered" padding="lg">
            ...
            >
              ...
              <Switch
                value={enableRemindersToggle}
                onValueChange={setEnableRemindersToggle}
