...
import { Heading, Body, DataText } from '@/components/ui/typography';
    ...
    >
      ...
      {goals.length === 0 ? (
        ...
        goals.map((goal, index) => {
          const goalHabits = habits.filter((h) => h.goalId === goal.id && h.isActive);
          ...
          const progress = goalHabits.length > 0 ? completedToday / goalHabits.length : 0;

          return (
            <Animated.View key={goal.id} entering={FadeInDown.duration(300).delay(index * 80)}>
              <Card variant="filled" padding="lg">
                <View style={{ gap: Spacing.lg }}>
                  ...
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <ProgressRing progress={progress} size={50} strokeWidth={4} labelSize="sm" />
                  </View>

                  {/* Habits list */}
                  {goalHabits.length > 0 && (
                    <View style={{ gap: Spacing.sm }}>
                      <Body size="xs" secondary style={{ textTransform: 'uppercase', letterSpacing: 1 }}>
                        {goalHabits.length} habit{goalHabits.length !== 1 ? 's' : ''} · {completedToday} done today
                      </Body>
                      {goalHabits.map((h) => (
                        <View
                          key={h.id}
                        ...
                        >
                          <View
                            style={{
                              ...
                              height: 6,
                              borderRadius: 3,
                              backgroundColor: logs.find((l) => l.habitId === h.id && l.date === today)
                                ? Colors.success
                                : Colors.border,
                            }}
                          />
