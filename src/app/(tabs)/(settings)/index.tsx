...
export default function SettingsScreen() {
  ...
  const [hapticsEnabled, setHapticsEnabled] = useStorage(
    "hapticsEnabled",
    true,
  );

  const [appTheme, setAppTheme] = useStorage<"system" | "light" | "dark">(
    "appTheme",
    "system",
  );
  ...
  const handleToggleReminders = async (value: boolean) => {
    if (value) {
      // Give habits without times a default morning nudge so the toggle does something.
      ...
      const granted = await enableReminders(latestHabits);
      if (!granted) {
        ...
        <SettingsGroup label="App Settings">
          <SettingsRow
            ...
            trailing={
              ...
              >
                <Body secondary style={{ textTransform: "capitalize" }}>
                  {appTheme}
                </Body>
                <SettingsChevron />
              </View>
